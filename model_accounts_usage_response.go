/*
InsightOps REST API

### Overview  Our REST API lets you use InsightOps through HTTP requests. Currently, the REST API allows you to perform the majority of the actions available through the UI, and has some additional functionality that is not available through the UI. You may use this API to automate common tasks (for example, via shell scripts), and to generally interact with InsightOps programmatically.  This page precisely describes the REST API and serves as a reference for the API. Each HTTP method and each URL endpoint is documented in a self-contained unit so that users only need to read about the HTTP methods relevant to them.  ### Terminology  * A **log entry** is an individual log event. * A **log** is a collection of log entries, or a single log stream. * A **log set** is a logical-only collection of logs, i.e. logs can be in multiple logsets and deleting a logset only deletes the relation between logs, not the logs themselves. * [Log Entry Query Language](https://docs.rapid7.com/insightops/log-search) (**LEQL**) is the query language used in Insight Ops to search log data.

API version: latest
Contact: support@rapid7.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package insightops

import (
	"encoding/json"
)

// checks if the AccountsUsageResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AccountsUsageResponse{}

// AccountsUsageResponse struct for AccountsUsageResponse
type AccountsUsageResponse struct {
	DailyUsage *AccountsUsageResponseDailyUsage `json:"daily_usage,omitempty"`
	// The total number of bytes written to every log, for the entire time range.
	PeriodUsage *int                         `json:"period_usage,omitempty"`
	Period      *AccountsUsageResponsePeriod `json:"period,omitempty"`
	// The account resource ID (UUID). A unique identifier for your organization.
	Id *string `json:"id,omitempty"`
	// The name of your organization.
	Name *string `json:"name,omitempty"`
}

// NewAccountsUsageResponse instantiates a new AccountsUsageResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAccountsUsageResponse() *AccountsUsageResponse {
	this := AccountsUsageResponse{}
	return &this
}

// NewAccountsUsageResponseWithDefaults instantiates a new AccountsUsageResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAccountsUsageResponseWithDefaults() *AccountsUsageResponse {
	this := AccountsUsageResponse{}
	return &this
}

// GetDailyUsage returns the DailyUsage field value if set, zero value otherwise.
func (o *AccountsUsageResponse) GetDailyUsage() AccountsUsageResponseDailyUsage {
	if o == nil || IsNil(o.DailyUsage) {
		var ret AccountsUsageResponseDailyUsage
		return ret
	}
	return *o.DailyUsage
}

// GetDailyUsageOk returns a tuple with the DailyUsage field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountsUsageResponse) GetDailyUsageOk() (*AccountsUsageResponseDailyUsage, bool) {
	if o == nil || IsNil(o.DailyUsage) {
		return nil, false
	}
	return o.DailyUsage, true
}

// HasDailyUsage returns a boolean if a field has been set.
func (o *AccountsUsageResponse) HasDailyUsage() bool {
	if o != nil && !IsNil(o.DailyUsage) {
		return true
	}

	return false
}

// SetDailyUsage gets a reference to the given AccountsUsageResponseDailyUsage and assigns it to the DailyUsage field.
func (o *AccountsUsageResponse) SetDailyUsage(v AccountsUsageResponseDailyUsage) {
	o.DailyUsage = &v
}

// GetPeriodUsage returns the PeriodUsage field value if set, zero value otherwise.
func (o *AccountsUsageResponse) GetPeriodUsage() int {
	if o == nil || IsNil(o.PeriodUsage) {
		var ret int
		return ret
	}
	return *o.PeriodUsage
}

// GetPeriodUsageOk returns a tuple with the PeriodUsage field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountsUsageResponse) GetPeriodUsageOk() (*int, bool) {
	if o == nil || IsNil(o.PeriodUsage) {
		return nil, false
	}
	return o.PeriodUsage, true
}

// HasPeriodUsage returns a boolean if a field has been set.
func (o *AccountsUsageResponse) HasPeriodUsage() bool {
	if o != nil && !IsNil(o.PeriodUsage) {
		return true
	}

	return false
}

// SetPeriodUsage gets a reference to the given int and assigns it to the PeriodUsage field.
func (o *AccountsUsageResponse) SetPeriodUsage(v int) {
	o.PeriodUsage = &v
}

// GetPeriod returns the Period field value if set, zero value otherwise.
func (o *AccountsUsageResponse) GetPeriod() AccountsUsageResponsePeriod {
	if o == nil || IsNil(o.Period) {
		var ret AccountsUsageResponsePeriod
		return ret
	}
	return *o.Period
}

// GetPeriodOk returns a tuple with the Period field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountsUsageResponse) GetPeriodOk() (*AccountsUsageResponsePeriod, bool) {
	if o == nil || IsNil(o.Period) {
		return nil, false
	}
	return o.Period, true
}

// HasPeriod returns a boolean if a field has been set.
func (o *AccountsUsageResponse) HasPeriod() bool {
	if o != nil && !IsNil(o.Period) {
		return true
	}

	return false
}

// SetPeriod gets a reference to the given AccountsUsageResponsePeriod and assigns it to the Period field.
func (o *AccountsUsageResponse) SetPeriod(v AccountsUsageResponsePeriod) {
	o.Period = &v
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *AccountsUsageResponse) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountsUsageResponse) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *AccountsUsageResponse) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *AccountsUsageResponse) SetId(v string) {
	o.Id = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *AccountsUsageResponse) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountsUsageResponse) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *AccountsUsageResponse) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *AccountsUsageResponse) SetName(v string) {
	o.Name = &v
}

func (o AccountsUsageResponse) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AccountsUsageResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.DailyUsage) {
		toSerialize["daily_usage"] = o.DailyUsage
	}
	if !IsNil(o.PeriodUsage) {
		toSerialize["period_usage"] = o.PeriodUsage
	}
	if !IsNil(o.Period) {
		toSerialize["period"] = o.Period
	}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	return toSerialize, nil
}

type NullableAccountsUsageResponse struct {
	value *AccountsUsageResponse
	isSet bool
}

func (v NullableAccountsUsageResponse) Get() *AccountsUsageResponse {
	return v.value
}

func (v *NullableAccountsUsageResponse) Set(val *AccountsUsageResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableAccountsUsageResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableAccountsUsageResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAccountsUsageResponse(val *AccountsUsageResponse) *NullableAccountsUsageResponse {
	return &NullableAccountsUsageResponse{value: val, isSet: true}
}

func (v NullableAccountsUsageResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAccountsUsageResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
